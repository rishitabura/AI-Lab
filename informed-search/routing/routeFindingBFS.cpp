...
vector<string> greedyBestFirstSearch(Node *start, Node * goal, map<Node*, int>& distances)
{
    // min priority queue
    ...
    while (!pq.empty())
    {
        ...
        for (const auto &edge : currentNode->adjacencies)
        {
            ...
            {
                pq.push(make_pair(neighborHeuristic, neighborNode));
...
int main()
{
    ...
    vector<string> path = greedyBestFirstSearch(n1, n13, distances);
